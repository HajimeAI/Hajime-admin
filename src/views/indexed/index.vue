    ...
    <PureTableBar title="List" :columns="columns" @refresh="onSearch">
      ...
      <template v-slot="{ size, dynamicColumns }">
        <pure-table stripe border align-whole="center" showOverflowTooltip table-layout="auto" :loading="loading"
          ...
          <template #status="{ row }">
            <el-tag :type="Number(row.enable) ? 'success' : 'danger'">{{
              Number(row.enable) ? "Public" : "Private"
              }}</el-tag>
          </template>
          ...
          <template #operation="{ row }">
            ...
            <el-popconfirm :title="`Are you sure to delete the data of ${row.name}?`" @confirm="handleDelete(row)">
              <template #reference>
